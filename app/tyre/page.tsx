...
export default function Tyre() {
  ...
  const fetchStints = async () => {
    ...
    const sessionKey = grandPrixMap[Object.keys(grandPrixMap).find(key => key.toLowerCase() === lowerCaseGrandPrix) || ""];
    ...
    const res = await fetch(`https://api.openf1.org/v1/stints?session_key=${sessionKey}`);
    const data = await res.json();

    setStints(data);
    const uniqueDrivers = [...new Set<number>(data.map((stint: Stint) => stint.driver_number))];
    setDrivers(uniqueDrivers);

    const calculatedTotalLaps = Math.max(...data.map((stint: Stint) => stint.lap_end));
  ...
  const ticks = totalLaps > 0 ? Array.from({ length: tickCount + 1 }, (_, i) => i * tickStep + 1) : [];
    ...
    <Card className="w-full max-w-4xl mx-auto mt-10 p-4 shadow-lg">
      <CardContent>
        ...
        <svg ref={svgRef} width="100%" height={drivers.length * 40 + 100}>
          {drivers.map((driver, rowIndex) => (
            <Group key={driver} top={rowIndex * 40 + 70}>
              ...
              <text x={5} y={30} fontSize={12} fill="black">{`#${driver}`}</text>
            </Group>
          ))}
          ...
          <g transform={`translate(0, ${drivers.length * 40 + 80})`}>
            ...
            {ticks.map((tick) => (
              <g key={tick} transform={`translate(${xScale(tick)}, 0)`}>
                ...
                <text x={0} y={20} fontSize={12} fill="black" textAnchor="middle">{tick}</text>
              </g>
            ))}
          </g>
        </svg>
        ...
        <div className="flex flex-col items-start mt-4">
          <h3 className="font-bold mb-2">Tire Compound Legend:</h3>
          {Object.entries(compoundColors).map(([compound, color]) => (
            <div key={compound} className="flex items-center mb-1">
              <div
                style={{
                  width: 20,
                  height: 20,
